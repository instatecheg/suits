        ...
        <form @submit.prevent="createIssues" class="form-grid">
          ...
          <div class="form-actions">
            ...
            <button type="submit" :disabled="isCreating" class="btn">
              ...
              {{ isCreating ? (rtl ? 'جاري الإضافة...' : 'Creating...') : (ui.rtl  ? 'إضافة الدعوى' : 'Add Case') }}
            </button>
          </div>
        </form>
      </section>
          ...
          <table class="cases-table">
            <thead>
              <tr>
                ...
                <th>إسم العميل</th>
                <th>إسم الخصم</th>
                <th>{{ ui.rtl  ? 'الإجراءات' : 'Actions' }}</th>
              </tr>
            </thead>
            <tbody>
                ...
                <td>
                  <div class="flex flex-wrap gap-2 justify-end">
                    <button class="btn" @click="viewSuit(suit.name)">
                      <i class="fas fa-eye"></i> {{ ui.rtl ? 'عرض' : 'View' }}
                    </button>

                    <button class="btn" @click="editSuit(suit.name)">
                      <i class="fas fa-edit"></i> {{ ui.rtl ? 'تعديل' : 'Edit' }} 
                    </button>

                    <button class="btn" :disabled="isDeleting" @click="deleteSuit(suit.name)">
                        <i class="fas fa-trash"></i> {{ ui.rtl ? 'حذف' : 'Delete' }}
                    </button>
